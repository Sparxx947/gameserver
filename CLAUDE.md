...
| Falle | Was passiert |
|---|---|
| `mkdir(mode=…)` | Der `mode` wird von der `umask` beschnitten. `chmod` muss **nach** `mkdir` kommen. Gilt auch für **Elternverzeichnisse**: `panel-aktion` setzt `umask 077`, und `mkdir(parents=True)` erzeugte darunter `0700 root` — der Container kam nicht mehr durch den Elternordner zu seinen eigenen Daten, Grafana lief in eine Neustartschleife, und die Installation meldete Erfolg (#266). |
