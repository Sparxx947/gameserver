...
Sieben allgemeine und **eines je Spielserver**, erzeugt von
`werkzeuge/statistik-dashboards.py` und bereitgestellt aus Dateien — nach einem
Neuaufbau sind sie damit wieder da, ohne dass jemand sie nachbaut:

| Dashboard | Was darauf steht | Quelle |
|---|---|---|
| **Maschine** | Last je Kern, Auslastung nach Art, Speicherverteilung, Wartedruck (PSI), Platte mit Hochrechnung, Inodes, I/O je Gerät, Netz, TCP, offene Dateien | node_exporter |
| **Spielserver** | Spieler je Server und Spielerstunden, Speicher, CPU, Netz, Platte und Wachstum, Zustandstabelle, Erreichbarkeit — mit Auswahlfeld für einzelne Server | Sammler, Probe |
| **Docker und Container** | CPU, Speicher gegen Grenze, I/O, Netz je Container, Neustarts in 24 h, jüngster Start, Verkehr über die Brücken, Prozesse je Container | cAdvisor |
| **Web und Zertifikat** | Anfragen je Sekunde und Code, Antwortzeiten p50/p90/p99, gleichzeitig offen, Upstreams, Neuladen, **Restlaufzeit des Zertifikats** | Caddy, Probe |
...
nach jeder Installation, jeder Entfernung und bei jedem `anwenden`. Wird ein
Server entfernt, verschwindet sein Dashboard mit ihm; eines für einen Server, den
es nicht mehr gibt, zeigt eine flache Linie und sieht aus wie ein stiller Server.

Wer auf **Statistik** klickt, landet auf der Liste der Dashboards, nicht auf
Grafanas Startseite — der Knopf im Panel führt direkt dorthin, und Grafanas
eigene Startseite ist zusätzlich auf die Liste gestellt.
...
> *Seven general dashboards and one per game server, generated and provisioned
> from files so a rebuild brings them back: machine (load per core, usage by
> mode, memory, pressure, disk with projection, I/O, network, TCP), game servers
> (players and player hours, usage, disk growth, state table, reachability, with
> a per-server selector), Docker and containers (cAdvisor), web and certificate
> (Caddy request rates, latency quantiles, certificate expiry), network and
> reachability, backup and disk, and the measurement itself. The per-server ones
> are created automatically from a template in the repository, filled in on the
> machine after every installation, removal and apply — and removed with their
> server, since a dashboard for a server that no longer exists shows a flat line
> and looks like a quiet one. Clicking "Statistik" lands on the list of
> dashboards rather than Grafana's start page. Gaps are never bridged, as in the
> panel's own history.*
...
```bash
werkzeuge/statistik-dashboards.py --daten gameserver
```

Sie führt **jede Abfrage jeder Tafel** gegen die laufende Instanz aus — durch
Grafana hindurch, nicht an ihm vorbei — und nennt die leeren. Eine Tafel ohne
Daten sieht aus wie ein ruhiger Messwert; genau so blieb unbemerkt, dass die
Datenquelle nach einer Umstellung auf einen Containernamen zeigte, den es nicht
mehr gab. Der Lauf muss am Ende sagen: *alle Abfragen liefern Daten*.

> *The check that matters runs every panel's query against the live instance —
> through Grafana rather than past it — and names the empty ones. An empty panel
> looks like a quiet measurement; that is exactly how a datasource pointing at a
> container name that no longer existed went unnoticed.*
